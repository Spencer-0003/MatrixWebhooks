...
export default async function (server: FastifyInstance) {
  ...
  const send = (webhookClient: typeof client, roomId: string, body: string): void => {
    webhookClient
      .sendMessage(roomId, {
        ...
        formatted_body: converter.makeHtml(body)
      })
  ...
  server.delete('/:token', async (req, res) => {
    ...
    if (
      !webhook ||
      (webhook.secret && (req.body as Record<string, string>).secret !== webhook.secret)
    )
      return res.send({
        error: !webhook ? 'Invalid webhook' : 'This webhook is protected with a secret.'
      });
